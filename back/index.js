...
app.get("/comandaAndroid", async (req, res) => {
  ...
  try {
    connection = await connectToDatabase();
    const [rows] = await connection.execute(
      "SELECT ID as id, ESTAT as estatus, PREU_TOTAL as total, PRODUCTES as produtos FROM comanda;"
    );
...
app.post("/comandaAndroid", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Failed to create comanda: ${error.message}` });
  } finally {
    if (connection) {
      await connection.end();
      console.log("Database connection closed.");
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const [userResult] = await connection.execute(
      "SELECT correo, password FROM usuari WHERE correo = ?",
      [correo]
    );
    ...
    if (!passwordMatch) {
      ...
      return res
        .status(200)
        .json({ message: "Login exitoso", userId: usuari.id, token });
    }
  } catch (error) {
